...
def start(message):
    bot.send_message(MY_ID, "🚀 بوت GitHub Actions جاهز!\nأرسل الرابط وسأعرض لك العداد ثانية بثانية.")
...
def process_video(message, url):
    try:
        ...
        for line in proc_pull.stdout:
            ...
            if match and (time.time() - last_update >= 5):
        ...
        # --- المرحلة 2: الضغط مع عداد ---
        bot.edit_message_text("⚙️ **المرحلة 2: جاري الضغط (HEVC)...**", MY_ID, status_msg.message_id)
        cmd_comp = ['ffmpeg', '-y', '-i', raw_file, '-c:v', 'libx265', '-crf', '28', '-preset', 'ultrafast', '-c:a', 'aac', final_file]
        ...
        for line in proc_comp.stdout:
            ...
            if match and (time.time() - last_update >= 5):
                ...
                curr = h*3600 + m*60 + s
                p = min(int((curr/total_seconds)*100), 100)
                bot.edit_message_text(f"⚙️ **جاري الضغط:**\n✅ المنجز: {curr}/{total_seconds} ثانية ({p}%)", MY_ID, status_msg.message_id)
                last_update = time.time()
        proc_comp.wait()

        # إرسال الفيديو
        size = os.path.getsize(final_file) / (1024*1024)
        bot.send_message(MY_ID, f"✅ اكتمل المهمة! الحجم: {size:.2f} MB")
